...
%>
<%
  userInfo= UserInformationDao.getUserInfo(request.getParameter("user"));
  userType = UserDao.getUserType(new User(request.getParameter("user"),null,0));
  ...
  <table class="table">
    ...
    <td>
      <select class="inputText" name="userType">
        <option value="0" <%=userType==0?"selected":""%>>用户</option>
        <option value="1" <%=userType==1?"selected":""%>>管理员</option>
      </select></td>
  </tr>
  </table>
